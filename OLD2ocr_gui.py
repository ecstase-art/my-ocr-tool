...
# ===== КОРРЕКЦИЯ ОРИЕНТАЦИИ ИЗОБРАЖЕНИЙ (автоматическая) =====
def correct_image_orientation(image_path):
    """
    Определяет ориентацию изображения с помощью Tesseract (0, 90, 180, 270)
    и поворачивает при необходимости.
    Возвращает путь к исправленному временному файлу или оригинал.
    """
    try:
        import cv2
        import pytesseract
    ...
    try:
        ...
        if angle == 180:
            ...
            return image_path  # 0° или неопределено
...
class OCRApp:
    ...
    def create_widgets(self):
        ...
        tk.Label(self.root, text="Лог обработки:").grid(row=3, column=0, sticky="w", padx=10)
    ...
    def process(self, input_dir, project_name):
        try:
            ...
            self.log(f"📄 Найдено новых: {len(entries)}")
            self.process_entries(entries, output_dir, checkpoint_path, processed)
    ...
    def process_entries(self, entries, output_dir, checkpoint_path, processed):
        if not entries:
            return
    ...
    def upload_files_with_retry(self, file_paths, upload_urls):
        ...
        for path, url in zip(file_paths, upload_urls):
            ...
            use_path = path
            # Коррекция ориентации для изображений
